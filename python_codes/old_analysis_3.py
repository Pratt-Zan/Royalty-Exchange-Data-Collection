...
def column_has_any_highlight(table, col_idx):
    """Check if any content row in the given column has a non-None highlighted field."""
...
def fill_rows_28_to_41(step1_data, step2_data, file_exists_in_both):
    ...
    try:
        ...
        # Rows 29-30: Type (determine from content row highlights, not header)
        mc_highlighted = column_has_any_highlight(table, 1)
        sr_highlighted = column_has_any_highlight(table, 2)
...
def main():
    ...
    step1_dir = os.path.join(project_dir, "analysis", "old_step_1")
    step2_dir = os.path.join(project_dir, "analysis", "old_step_2")
    output_dir = os.path.join(project_dir, "analysis", "old_step_3")
